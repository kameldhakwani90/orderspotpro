...
export const addUser = async (userData: Omit<User, 'id' | 'motDePasse'> & { motDePasse?: string }): Promise<User> => {
  ...
  const newUser: User = {
    ...userData,
    id: `user-${Date.now()}`,
    motDePasse: userData.motDePasse || '1234' // Default password if not provided
  };
  users.push(newUser);
  return newUser;
};
...
export const updateRoomOrTable = async (id: string, data: Partial<Omit<RoomOrTable, 'id' | 'urlPersonnalise' | 'hostId'>>): Promise<RoomOrTable | undefined> => {
  ...
  if (itemIndex > -1) {
    ...
    roomsOrTables[itemIndex] = {
        ...currentItem,
        ...data,
        globalSiteId: data.globalSiteId || currentItem.globalSiteId, // Ensure globalSiteId is preserved or updated
        parentLocationId: data.parentLocationId === undefined ? currentItem.parentLocationId : data.parentLocationId, // Allow setting parentLocationId to null/undefined
    };
...
export const deleteRoomOrTable = async (id: string): Promise<boolean> => {
    ...
    if (locationToDelete) {
        roomsOrTables = roomsOrTables.map(rt =>
            ...
            ? { ...rt, parentLocationId: locationToDelete.parentLocationId }
            : rt
        );
    }
...
export const addServiceCategory = async (data: Omit<ServiceCategory, 'id' | 'data-ai-hint'>): Promise<ServiceCategory> => {
  const newCategory: ServiceCategory = { ...data, id: `cat-${Date.now()}`, "data-ai-hint": data.nom.toLowerCase().substring(0,15) };
...
export const updateServiceCategory = async (id: string, data: Partial<Omit<ServiceCategory, 'id' | 'hostId' | 'data-ai-hint'>>): Promise<ServiceCategory | undefined> => {
  ...
  if (catIndex > -1) {
    serviceCategories[catIndex] = { ...serviceCategories[catIndex], ...data, "data-ai-hint": data.nom ? data.nom.toLowerCase().substring(0,15) : serviceCategories[catIndex]["data-ai-hint"] };
...
export const addService = async (data: Omit<Service, 'id' | 'data-ai-hint'>): Promise<Service> => {
  const newService: Service = { ...data, id: `svc-${Date.now()}`, "data-ai-hint": data.titre.toLowerCase().substring(0,15) };
...
export const updateService = async (id: string, data: Partial<Omit<Service, 'id' | 'hostId' | 'data-ai-hint'>>): Promise<Service | undefined> => {
  ...
  if (serviceIndex > -1) {
    services[serviceIndex] = { ...services[serviceIndex], ...data, "data-ai-hint": data.titre ? data.titre.toLowerCase().substring(0,15) : services[serviceIndex]["data-ai-hint"] };
