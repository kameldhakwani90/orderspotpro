  ...
  try {
    ...
    for (const model of models.reverse()) {
      try {
        ...
        console.warn(\`  ⚠️  \${model}: erreur suppression (peut-être vide)\`);
      }
    }
...
export async function getDatabaseStats() {
  try {
    ...
    ${Array.from(interfaces.keys()).map(modelName => {
      const camelName = modelName.charAt(0).toLowerCase() + modelName.slice(1);
      return `    try {
      stats.${camelName}Count = await prisma.${camelName}.count();
    } catch (error) {
      stats.${camelName}Count = 0;
    }`;
    }).join('\n')}
...
// ====================================
// EXÉCUTION PRINCIPALE AVEC GESTION D'ERREURS ROBUSTE
// ====================================
...
try {
  ...
  if (interfaces.size === 0) {
    ...
    // Essayer de lire le contenu pour diagnostic
    if (fs.existsSync(typesPath)) {
      const content = fs.readFileSync(typesPath, 'utf-8');
      console.log('📋 Aperçu du contenu de types.ts:');
  ...
  if (fs.existsSync(servicePath)) {
    ...
    console.log(`📊 Taille du fichier généré: ${size} bytes`);
    
    if (size > 1000) {
      console.log('✅ Fichier prisma-service.ts généré avec succès et de taille normale');
    } else {
      console.warn('⚠️  Fichier prisma-service.ts généré mais semble petit');
    }
  }
  ...
  console.error('❌ Erreur critique dans generatePrismaServiceFromData:');
  ...
  console.log(`- data.ts existe: ${fs.existsSync(dataPath)}`);
  console.log(`- Répertoire service existe: ${fs.existsSync(path.dirname(servicePath))}`);
